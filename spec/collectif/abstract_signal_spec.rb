...
describe Collectif::AbstractSignal do

  class MySignal < Collectif::AbstractSignal
  ...
  describe "#receive" do
    after :each do
      Collectif.emitter.remove_listener name
    ...
    it "receives events for this signal with their payload" do
      ...
      Collectif.emitter.emit(name, emitted_payload)
    ...
    context "when a version is not specified" do
      ...
      it "receives events transmitted using only the signal's name" do
        ...
        expect { Collectif.emitter.emit name }.to change { counter }.by(1)
      end

      it "stores the event in the consumer" do
        signal.receive {}
        expect(Collectif.emitter.events).to include(name: name, version: version)
      end
    ...
    context "when a version is specified" do
      ...
      it "receives events transmitted using the signal's name and version" do
        ...
        expect { Collectif.emitter.emit "#{name}:#{version}" }.to change { counter }.by(1)
      end
    ...
    context "with a wildcard signal" do
      ...
      it "receives events for all signals signal with their payloads" do
        ...
        Collectif.emitter.emit(Collectif::Emitter::WILDCARD, foo: "bar")
        Collectif.emitter.emit(Collectif::Emitter::WILDCARD, foo: "baz")
