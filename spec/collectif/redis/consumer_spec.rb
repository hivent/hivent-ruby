...
describe Collectif::Redis::Consumer do
  ...
  after :each do
    ...
    Collectif.emitter.__events.clear
  end
  ...
  describe "#consume" do
    ...
    let(:producer) { Collectif::Redis::Producer.new(redis) }
    ...
    context "when there are items ready to be consumed" do
      ...
      it "emits the item with indifferent access" do
        Collectif.emitter.on(event[:meta][:name]) do |received|
      ...
      it "emits the item with name only and name with version" do
        counter = 0
        Collectif.emitter.on(event_name_with_version) do |_|
          counter += 1
        end
        Collectif.emitter.on(event[:meta][:name]) do |_|
          counter += 1
        end
      ...
      context "when several items are produced" do
        ...
        it "consumes all events in the order they were produced" do
          events = []
          Collectif.emitter.on(event_name_with_version) do |event|
            events << event
          end
    ...
    describe "Wildcard event consumption" do
      ...
      it "consumes all events" do
        counter = 0
        Collectif.emitter.on(Collectif::Emitter::WILDCARD) do |_|
          counter += 1
        end
